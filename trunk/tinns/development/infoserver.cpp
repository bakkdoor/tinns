...
/*
	...
	REASON: - added little fix (Config->GetOption("server ip"))
...
PInfoServer::~PInfoServer()
{
	if(mListener != NL_INVALID)
	{
	    ...
	    NLboolean nlanswer;
		nlanswer = nlClose(mListener);
		if(nlanswer == NL_TRUE)
		{
		    Console->Print("NL ERROR: %s NL SYS: %s", nlGetErrorStr(nlGetError()), nlGetSystemErrorStr(nlGetSystemError()));
		}
		mListener=NL_INVALID;
	}
...
bool PInfoServer::HandleServerList(PClient *Client, const u8 *Packet, int PacketSize)
{
	...
	{
		const std::string &ServerName = Config->GetOption("server_name");
		u16 len = sizeof(SERVERLIST)+ServerName.length()+1;
		*(u16*)&SERVERLIST[1]=len-3;
		SERVERLIST[17]=(u8)len+1;

		u32 dip = IPStringToDWord(Config->GetOption("server_ip").c_str());
