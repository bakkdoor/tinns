...
const int NSPROTO_IPX = 0;
const int NSPROTO_SPXII = 0;
int sockerrno;
...
static NLboolean ipx_GetHostAddress(NLaddress *address)
{
    ...
    sock = socket(PF_IPX, SOCK_DGRAM, 0);
...
void ipx_Close(NLsocket socket)
{
    ...
    if((sock->type == NL_RELIABLE_PACKETS || sock->type == NL_RELIABLE) && sock->listen == NL_FALSE)
    {
        ...
        (void)setsockopt((SOCKET)sock->realsocket, SOL_SOCKET, SO_LINGER, (const char *)&l, sizeof(l));
    }
...
NLint ipx_bind(SOCKET socket, const struct sockaddr *a, int len) {return 0;}
