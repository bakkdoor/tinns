...
PGameDefs::~PGameDefs()
{
  ...
  for ( PDefRespawnMap::iterator i = mRespawnDefs.begin(); i != mRespawnDefs.end(); i++ )
    delete i->second;
  for ( PDefWeaponMap::iterator i = mWeaponDefs.begin(); i != mWeaponDefs.end(); i++ )
    delete i->second;
...
bool PGameDefs::LoadWeaponDefs()
{
  PDefParser parser;
  int nDefs = 0, nErrors = 0;
  const string DEF_FILE = DEF_WEAPONS;
  ...
  if ( parser.Parse( DEF_FILE.c_str() ) )
  {
    ...
    for ( PDefTokenList::const_iterator i = t.begin(); i != t.end(); i++ )
    {
      PDefWeapon *it = new PDefWeapon();
      bool loadfail = !it->LoadFromDef( *i ), insertfail = false;

      if ( !loadfail )
        insertfail = !mWeaponDefs.insert( std::make_pair( it->GetIndex(), it ) ).second;
      if ( loadfail || insertfail )
      {
        if ( insertfail )
          Console->Print( "Weapons def error (duplicate id %i): %s", it->GetIndex(), it->GetName().c_str() );
        else
          Console->Print( "Weapons def load error @ %i", nDefs + nErrors );
        ++nErrors;
        delete it;
      }
      else
  ...
  {
    Console->Print( "%s Error loading weapons defs", Console->ColorText( RED, BLACK, "[ERROR]" ) );

    return ( false );
  }
  if ( nErrors > 0 )
    Console->Print( "%s Loaded %i weapons defs, %i error(s).", Console->ColorText( RED, BLACK, "[ERROR]" ), nDefs, nErrors );
  else
    Console->Print( "%s Loaded %i weapons defs, %i error(s).", Console->ColorText( GREEN, BLACK, "[Success]" ), nDefs, nErrors );
...
const PDefWeapon* PGameDefs::GetWeaponDef( int Index ) const
{
  PDefWeapon *Result = 0;
  PDefWeaponMap::const_iterator i = mWeaponDefs.find( Index );
  if ( i != mWeaponDefs.end() )
    Result = i->second;
