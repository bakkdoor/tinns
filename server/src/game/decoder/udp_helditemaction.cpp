...
bool PUdpHeldItemAction::DoAction()
{
  ...
  if ( gDevDebug )
    Console->Print( "%s Handled item action toward target %d (0x%08x) height %d/15 score %d/255", Console->ColorText( CYAN, BLACK, "[DEBUG]" ), mTargetRawItemID, mTargetRawItemID, mTargetedHeight, mScore );
